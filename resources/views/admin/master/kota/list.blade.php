        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body px-5 py-4">
                    <div class="row pt-3">
                        <div class="col-md-6">
                            <h5>Daftar List Kota</h5>
                        </div>
                        <div class="col-md-6 text-end">
                            <a href="{{ route('admin.master.kota.add_view') }}" class="btn btn-outline-primary w-200px">
                                ...
                                <tbody>
                                    @foreach ($data as $key => $value)
                                        <tr>
                                            ...
                                            <td class="text-center"><small>{{ $value->pulau->name }}</small></td>
                                            <td class="text-center"><small>{{ $value->provinsi->name }}</small></td>
                                            <td class="text-center"><small>{{ $value->name }}</small></td>
                                            <td class="text-center">
                                                <div class="row">
                                                    <div class="col">
                                                        <a href="{{ route('admin.master.kota.edit_view', ['id_kota' => $value->id_kota]) }}" class="btn btn-sm btn-outline-primary w-100px">
                                                            <small>Ubah Kota</small>
                                                        </a>
                                                    </div>
                                                </div>
                                                <div class="row mt-1">
                                                    <div class="col-md-12">
                                                        <button type="button" class="btn btn-sm btn-outline-secondary w-100px" onclick="show_modal_hapus('{{ $value->id_kota }}', '{{ $value->name }}', '{{ $value->image_path }}')">
...
<script>
    ...
    function show_modal_hapus(id_kota, name, image_path){
        var url = "{{ route('admin.master.kota.delete', ['id_kota' => ':id_kota']) }}";
        url = url.replace(':id_kota', id_kota);
